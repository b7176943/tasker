//GOOGLE FORMS CANT INCLUDE NEWLINES!!
...
function csvToObj(csv) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (line) {
            ...
            obj.Tags = replaceTags(obj.Tags.toLowerCase().split(",")).map(item=>item.trim());
            obj.BodyParts = obj.BodyParts.toLowerCase().split(",").map(item=>item.trim());
            obj.Toys = replaceToys(obj.Toys.toLowerCase().split(",")).map(item => item.trim());
...
function replaceTags(array) {
    for (var i = 0; i < array.length; i++) {
        for (var j = 0; j < Tags.length; j++) {
            array[i] = array[i].replace(Tags[j].title.toLowerCase(), Tags[j].id);
        }
    }
    return array;
}
function replaceToys(array) {
    for (var i = 0; i < array.length; i++) {
        for (var j = 0; j < Toys.length; j++) {
            array[i] = array[i].replace(Toys[j].title.toLowerCase(), Toys[j].id);
        }
    }
    return array;
}
...
function nextTask() {
    ...
    if (task) {
        ...
        if (enableTimer && task.Minutes != "" && task.Minutes > 0) {
            ...
            if (currentTask.Type == "Time limit") {
